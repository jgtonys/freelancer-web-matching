...
class ModifyUser extends React.Component {
  ...
  componentDidMount() {
    ...
    if(userType == 'FREELANCER') this.getFreelancerInfo();
    else if(userType == 'CLIENT') this.getClientInfo();
  }
  getClientInfo = async () => {
    let id = window.sessionStorage.getItem('uid');
    const d = await service.getClientInfo(id).then(r => {
      return r.data;
    })
    .catch(e => {
    ...
    this.setState({
      user_id: this.state.getInfo.user_id,
      password: this.state.getInfo.password,
      name: this.state.getInfo.name,
      phonemask: this.state.getInfo.phone,
    })
  }

  getFreelancerInfo = async () => {
    let id = window.sessionStorage.getItem('uid');
    const d = await service.getFreelancerInfo(id).then(r => {
      return r.data;
    })
    .catch(e => {
      alert(e.response.data.message);
    ...
    this.setState({
      ...
      user_id: this.state.getInfo[0].user_id,
      password: this.state.getInfo[0].password,
      name: this.state.getInfo[0].name,
      phonemask: this.state.getInfo[0].phone,
    })
  }
  ...
  addUser = event => {
    ...
    else if(this.state.userType == 'FREELANCER') { //freelancer
      let id = window.sessionStorage.getItem('uid');
        ...
        <Paper className={classes.paper}>
          <form onSubmit={this.addUser} className={classes.form}>
            <FormControl margin="normal" required fullWidth>
              ...
              <Input id="userId" name="userId" value={this.state.user_id} onChange={this.handleChange('user_id')}/>
            </FormControl>
              ...
              <Grid container spacing={24}>
                <Grid item xs={8}>
                  <FormControl margin="normal" required fullWidth>
                    ...
                    <Input value={this.state.password} disabled/>
                  </FormControl>
                </Grid>
                <Grid item xs={4}>
            ...
            <FormControl margin="normal" required fullWidth>
              ...
              <Input name="userName" id="userName" value={this.state.name} onChange={this.handleChange('name')}/>
            </FormControl>
            <FormControl margin="normal" required fullWidth>
